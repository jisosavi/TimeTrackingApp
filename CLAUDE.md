...
Single PostgreSQL database, hosted on Railway. The `DATABASE_URL` in a local `.env` is **not** necessarily the instance the deployed backend uses — verify before assuming a schema change or a row is visible to the running app (as of Sep 2026 the local one was three months stale). `backend/lib/db.ts` exports one `sql` tagged-template client (postgres.js). Schema is applied at startup by `backend/lib/migrate.ts` which runs any unapplied `.sql` files from `backend/migrations/` in order. To add a column, create a new numbered migration file there.
