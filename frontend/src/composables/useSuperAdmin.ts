...
export function useSuperAdmin() {
  ...
  async function fetchCompanies() {
    ...
    error.value = null
    try {
      const data = await get<{ companies: Company[] }>('/api/companies')
  ...
  async function updateCompany(
    ...
    const data = await post<{ company: Company }>('/api/companies', { id, ...fields })
  ...
  async function fetchBusinessId(companyId: number): Promise<string> {
    const data = await get<{ business_id: string }>(`/api/fetch_business_id?company_id=${companyId}`)
    return data.business_id
  }

  async function fetchCompanyAdmins(companyId: number): Promise<CompanyAdmin[]> {
    const data = await get<{ admins: CompanyAdmin[] }>(`/api/company_admins?company_id=${companyId}`)
    return data.admins
  }

  async function saveAdmin(companyId: number, admin: { id: number; email: string; name: string; password: string }): Promise<void> {
    await post('/api/company_admins', { company_id: companyId, ...admin })
  }

  async function deleteAdmin(companyId: number, adminId: number): Promise<void> {
    await del('/api/company_admins', { company_id: companyId, id: adminId })
  }
